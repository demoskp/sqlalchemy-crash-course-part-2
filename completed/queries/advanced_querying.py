...
from completed.models import User, Drive, Distance, Duration, Unit, Role
...
users_with_avg_drive_distance_or_0 = (
    ...
    .outerjoin(User.drives)
    .outerjoin(Drive.distance)
    .add_columns(func.coalesce(func.avg(Distance.value), 0).label("avg_drive_avg_drive_distance"))
    .group_by(User.id)
    .all()
)
...
users_with_avg_drive_distance_or_0_including_null = (
    User
    .query
    .outerjoin(User.drives)
    .outerjoin(Drive.distance)
...
print(users_with_avg_drive_distance_or_0_including_null)
